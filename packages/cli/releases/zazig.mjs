...
function hasPinnedBuild() {
  return existsSync7(join8(CURRENT, "packages", "local-agent", "dist", "index.js"));
}
...
async function start() {
  ...
  if (zazigEnv === "production" && hasPinnedBuild()) {
    ...
    agentEntryOverride = join9(buildDir, "packages", "local-agent", "dist", "index.js");
    const sha = getCurrentBuildSha();
...
async function runPromote(repoRoot, defaultBranch) {
  ...
  console.log("\nCommitting bundle...");
  try {
    execSync6("git add packages/cli/releases/zazig.mjs", { cwd: repoRoot, stdio: "pipe" });
    const diff = execSync6("git diff --cached --name-only", { encoding: "utf-8", cwd: repoRoot }).trim();
    if (diff) {
      execSync6('git commit -m "chore: update production CLI bundle"', { cwd: repoRoot, stdio: "pipe" });
