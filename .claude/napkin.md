...
| 2026-04-15 | Doppler project gone | Napkin + global CLAUDE.md reference Doppler project `zazig` — it no longer exists. Only `tom` and `trw-finances`. Wasted several minutes trying to fetch `SUPABASE_ACCESS_TOKEN` from it. | Check `doppler projects` first. If tom is missing Supabase keys too, either ask user for a token or write migrations defensively and hand off the Management API curl for the user to run. |
| 2026-04-15 | ideas RLS SELECT policy | Migration 028 adds `authenticated_read_own` SELECT policy on companies/machines/projects/features/jobs/events/messages/memory_chunks/company_roles — but NOT on `ideas`. Migration 130 only adds UPDATE. Webui works because someone must have direct-ALTERed a SELECT policy live (confirmed: live prod has `Users can view own company ideas` policy). Napkin line 112 hints "Phase 3 SQL done: ideas SELECT policy" but it's not in the migrations dir. | When adding auth-scoped reads for a table, check both the migrations dir AND be prepared for the live DB to diverge. Write the policy as `DROP POLICY IF EXISTS … CREATE POLICY …` so it's safe to reassert. Migration 233 does this. |
| 2026-04-15 | events.idea_id | `events` table schema: `id, company_id, job_id, machine_id, role, event_type, detail, created_at`. **No `idea_id` column.** `create-idea` edge function was inserting `idea_id: idea.id` which silently failed (no error check on the insert). No idea lifecycle events existed in `events` before this fix. | Idea references on `events` belong in `detail` jsonb: `detail: {idea_id, originator, source}`. Subscribers filter via `detail->>'idea_id'`. Fixed in create-idea 2026-04-15. |
...
| 2026-03-11 | OAuth tokens as API keys | ~~Tried using `sk-ant-oat01-...` as ANTHROPIC_API_KEY — refused.~~ **STALE (2026-03-23)**: OAuth tokens now work as ANTHROPIC_API_KEY. Verified with `curl` against `api.anthropic.com` — 200 OK. Keychain entry is `Claude Code-credentials` (service), not `claude-vscode`. |
...
| 2026-03-11 | expert_sessions RLS blocks daemon updates | `updateSessionStatus` silently failed — RLS only allowed `service_role` to UPDATE, daemon uses JWT (`authenticated` role). Session stayed `requested` forever, causing poll re-delivery loop. | Add UPDATE policy for `authenticated` role on `expert_sessions`. Also check `.select("id")` after `.update()` to detect 0-row matches (RLS silent denial). |
| 2026-03-11 | Poll re-delivery creates restart loop | Daemon picks up same session every 10s poll cycle, kills stale tmux, respawns — Claude never gets time to run. | Add `activeSessions.has(sessionId)` guard at top of `handleStartExpert`. Deduplication is essential when poll-based delivery doesn't ack. |
| 2026-03-11 | local-agent dist vs src build | Ran `npx tsc -p tsconfig.json` (noEmit: true) instead of `npm run build` (uses tsconfig.build.json). Dist files never updated. Bundle was stale. | Always use `npm run build` (which uses `tsconfig.build.json`) to produce dist files. The `tsconfig.json` is check-only (`noEmit: true`). |
