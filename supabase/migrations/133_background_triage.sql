-- 131_background_triage.sql
-- Background triage: add 'triaging' status, triage-analyst role, update request_standalone_work.

BEGIN;

-- 1. Add 'triaging' and 'workshop' to ideas status constraint
...
-- 3. Update request_standalone_work to allow triage-analyst (no feature creation needed)
...
BEGIN
  ...
  IF p_role IS NULL OR p_role NOT IN (
    ...
    RETURN jsonb_build_object(
      ...
      true,
      'reason',
      'role must be one of: pipeline-technician, monitoring-agent, verification-specialist, project-architect, triage-analyst'
    );
  END IF;
  ...
  ELSIF p_role = 'triage-analyst' THEN
    -- Triage-analyst operates on ideas, not features. No feature creation needed.
    v_feature_id := NULL;
  ...
  v_title := CASE p_role
    ...
    WHEN 'triage-analyst' THEN 'Triage idea'
  ...
  v_job_type := CASE p_role
    ...
    WHEN 'triage-analyst' THEN 'research'
    ELSE 'research'
  END;
