...
description: Coordinate a multi-agent build using Claude Code Agent Teams. Spawns a team with shared task list, contract-first pattern, mandatory Codex review. Use when a task crosses architectural layers (data/service/view) or needs parallel work. Invoke with /build-with-agent-team [plan-path] [num-agents] — or just describe what to build inline.
---

# Build with Agent Team

Coordinate a parallel multi-agent build using Claude Code's Agent Teams feature. The team lead orchestrates teammates via a shared task list, enforcing contract-first delivery so layers integrate cleanly.
...
Write `{project-dir}/.claude/cpo-report.md` with:
