# Exec Knowledge Architecture — Design Document (v4)

**Date:** 2026-02-22
**Status:** approved
**Authors:** Tom (owner), CPO (agent)
**Supersedes:** v3, v2, v1, sketch (all dated 2026-02-21 or 2026-02-20)
**Reviewed by:** CPO (Opus) interactive walkthrough, Codex (gpt-5.3-codex, xhigh reasoning), Gemini (gemini-3.1-pro-preview) — independent second opinions on all findings; adversarial review by Codex + Gemini (v3 review); second opinions on v4 revisions from both models
...
**Review docs:** `docs/plans/2026-02-21-exec-knowledge-architecture-v2-review.md`, `docs/plans/2026-02-21-exec-knowledge-architecture-v3-review.md` (adversarial), `docs/plans/2026-02-22-knowledge-v4-revisions.md`
...
The v1 design proposed a single "Knowledge Lenses" system with atomic claims. The v2 split that into two fundamentally different systems. This v4 incorporates all review findings — including independent second opinions from Codex and Gemini, adversarial review, and targeted revisions with second opinions — into the final architecture.
...
**Hallucination guard (v4, from adversarial review):** Tier 2 claim titles carry an explicit fetch instruction to prevent models from hallucinating claim content:
```
...
**Claim quality rubric (v4, from adversarial review):** Every claim must pass all criteria before `status = 'active'`:
...
8. Schedule compaction (v4): Deactivated chunks retained for 90 days.
...
**Chunk compaction strategy (v4, from adversarial review):** Inactive chunks accumulate during re-ingestion. A weekly `pg_cron` job hard-deletes unreferenced inactive chunks older than 90 days (measured from `deactivated_at`, not `created_at`). Deletes are batched at 1000 rows per run to avoid WAL bloat. Chunks referenced by `doctrine_canon_refs` are retained indefinitely for provenance preservation. This is a Phase 3+ concern — the strategy is stated here for architectural completeness.
...
> **Budget Accounting Note (v4):** The token economics table tracks two distinct budgets:
>
...
create table canon_sections (
  ...
  deactivated_at timestamptz,              -- set on soft-delete; compaction deletes after 90 days
  embedding vector(1536),
...
| **Injection-to-mention ratio** | % of injected claims the agent mentions or references | >50% |
...
**Definition of done:** Eval metrics (injection relevance@5, mention ratio, token efficiency) are being captured and meeting targets. RLS tenant isolation verified. Cache hit rate measured. **Causal A/B experiment passed** (see below). Note: in Phase 1, "relevance@5" measures proactive claim selection quality (no reactive search yet). Phase 2 extends this to tool-driven search recall.
...
**Estimated effort:** 6-8 days (was 5-7 in v2; added quality gate, sanitization, immutable chunks)
...
6. **Where are embeddings computed?** ~~Resolved in v4.~~ Edge Function calling OpenAI `text-embedding-3-small` API. Computed server-side on insert/update, stored in Postgres. Local agents never compute embeddings directly. This keeps the embedding pipeline centralized and consistent. If latency is an issue for bulk ingestion (Phase 3+), batch embedding via a background job.

7. **Canon licensing for marketplace:** Start with open/public sources only (YC content, regulatory texts, open-source docs) for the first canon libraries. Explore publisher licensing deals (like Audible/Blinkist) once the model is proven.

8. **Canon library pricing:** One-time purchase, included in subscription tier, or per-canon subscription for updates. Pricing should reflect curation value, not compute cost.
...
10. **Canon sanitization depth:** ~~Resolved in v4.~~  Two-layer defense: Layer 1 uses targeted delimiter stripping at ingestion (exact matches only, preserving angle brackets in code/HTML); Layer 2 uses nonce-based delimiters at runtime (`<zazig:canon nonce="...">`) that are unguessable per-request. HTML entity escaping explicitly rejected — it corrupts code snippets containing `<Button>`, `List<T>`, etc.

11. **Canon upload mechanism:** How do founders provide source documents for ingestion? CLI upload (`zazig canon ingest <file>`), dashboard file picker, Supabase Storage bucket, or URL-only? Needs design before Phase 3.
...
### v2 → v3 Changes (from review + second opinions)

| # | Change | Source |
|---|--------|--------|
| 1 | Downgraded Knowledge Curator from Phase 0.5 to claim authoring template | Codex + Gemini consensus |
| 2 | Added claim status state machine (draft/proposed/active/archived) with RLS enforcement | Original review + Gemini refinement |
| 3 | Parse quality gate elevated to Phase 3 blocker | Original review, unanimously confirmed |
| 4 | Cache-optimized prompt stack (static/dynamic split) elevated to hard requirement | Original review + Gemini elevation |
| 5 | Canon schema simplified for Phase 3 (4 tables); Phase 6 expansion (7 tables) | Original review + Codex refinement |
| 6 | Company-level scoping only in Phase 1; task-level deferred to Phase 4 | Original review |
| 7 | Immutable canon chunks for re-ingestion (replaces cascade triggers) | Gemini second opinion |
| 8 | Two-tier eval: deterministic telemetry (Phase 1) + LLM eval (Phase 2+) | Original review + Gemini refinement |
| 9 | Two-layer canon injection defense (ingestion sanitization + runtime sandboxing) | Codex second opinion + CPO refinement |
| 10 | Provenance/citation invariant as design principle | Codex second opinion |
| 11 | Tenant isolation as explicit invariant with RLS verification | Codex second opinion |
| 12 | Injection token budget formalized as hard cap | Gemini second opinion |
| 13 | Design principles reordered + expanded from 7 to 14 | Review consensus |
| 14 | "Why not unified?" counter-argument section added | Review consensus |

### Reviewer convergence (v3):
- **All three** (CPO, Codex, Gemini): Doctrine/canon split is correct. Progressive disclosure is right. Parse quality gate is non-negotiable. Prompt cache optimization is critical. Schema simplification for Phase 3 is correct. Hand-curation before automation.
- **Codex + Gemini** (CPO revised): Curator Phase 0.5 was over-engineered. Downgraded.
- **Gemini** (adopted): Immutable chunks, RLS over CHECK constraints, deterministic telemetry before LLM evals, injection budget as hard requirement.
- **Codex** (adopted): Provenance invariant, tenant isolation, prompt injection risk, eval gates per phase.

### v3 → v4 Changes (from adversarial review + revisions + second opinions)

| # | Revision | Change | Source |
|---|----------|--------|--------|
| A1 | Causal A/B experiment | Added blind pairwise comparison (50 paired tasks, 3+ roles, fixed temp/seed, sign test) as Phase 1→Phase 2 gate | Adversarial review; Codex refined scoring to pairwise; Gemini added stochasticity control |
| A2 | Tier 2 hallucination guard | Added `→ doctrine_read()` trace-based enforcement, `hallucinated_application` metric, `FORCE_TIER3_INJECTION` feature flag (20% threshold) | Adversarial review; Codex simplified to feature flag; Gemini added prompt reinforcement |
| A3 | Goodhart fix / metric rename | Renamed `utilization_ratio` → `mention_ratio` (diagnostic), added counterfactual sampling (Phase 2+), distinguished diagnostic vs quality metrics | Adversarial review; Codex added `hallucinated_application`; Gemini emphasized counterfactual as true metric |
| A4 | Delimiter escaping | Targeted stripping at ingestion (exact matches only), nonce-based delimiters as primary runtime defense, HTML escaping explicitly rejected | Adversarial review; Gemini identified code corruption risk (`<Button>`, `List<T>`); Codex proposed nonce approach |
| A5 | Claim quality rubric | 4 core criteria (actionable, specific, non-obvious, testable) + conditional 5th (verifiable+fresh for facts/policies), pass/fail examples | Adversarial review; Gemini added 5th criterion for fact-type claims |
| A6 | Token economics clarification | Budget accounting note, visual breakdown diagram, 2 worked examples with exact token totals | Adversarial review; Gemini proposed worked examples |
| A7 | Chunk compaction strategy | Added `deactivated_at` column (not `created_at`), `pg_cron` + `NOT EXISTS` batched deletes (LIMIT 1000), 90-day retention | Adversarial review; Codex fixed retention semantics + `NOT IN` → `NOT EXISTS`; Gemini proposed `pg_cron` |
| A8 | "What's Novel" section | Split into Technical Novelty (3 points with testable hypotheses) + Defensibility (4 points, Spotify analogy), softened absolute claims | Adversarial review; Codex proposed testable hypotheses; Gemini insisted on softening absolutes |

### Reviewer convergence (v4):
- **Codex + Gemini** (both): Targeted stripping over HTML escaping (A4). Pairwise comparison over absolute rubric (A1). `deactivated_at` over `created_at` for retention (A7). Counterfactual sampling as the real outcome metric (A3).
- **Codex** (adopted): Nonce-based delimiters, `NOT EXISTS` over `NOT IN`, `hallucinated_application` metric, testable hypotheses for novelty claims.
- **Gemini** (adopted): Code corruption risk from HTML escaping, stochasticity control for experiments, 5th rubric criterion, `pg_cron` for compaction, worked examples for token economics, softened absolutes.

### v4 Post-Review Fixes (from review-plan walkthrough)

| # | Type | Fix |
|---|------|-----|
| B1 | Bug | RRF SQL rewritten — rank computed per-CTE, not in fused CTE |
| B2 | Bug | Injection flow Step 5 delimiter `<canon-passage>` → `<zazig:canon>` |
| I1 | Inconsistency | Principle count "Twelve" → "Fourteen" |
| I2 | Inconsistency | Added `hallucinated_claim_ids` + `hallucinated_application_rate` to `knowledge_eval_log` schema |
| I3 | Inconsistency | Novelty claim softened ("are not aware" → "have not found") |
| G1 | Gap | Phase 1 effort updated to separate engineering (5-6d) from A/B experiment (1-2wk) |
| G2 | Gap | Resolved Open Question #6 — Edge Function calling OpenAI API |
| G3 | Gap | Renamed Phase 1 metric to "injection relevance@5" with clarifying note |
| G4 | Gap | Added Open Question #11 — canon upload mechanism |
| G5 | Gap | Specified FORCE_TIER3_INJECTION automation (weekly per-role, founder override) |
| G6 | Gap | Added embedding dimension lock-in note to Open Question #1 |
