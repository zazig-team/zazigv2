-- 077_standalone_feature_pipeline.sql
-- Route all standalone jobs through the feature pipeline.
-- When request_standalone_work is called without a feature_id, auto-create
-- a feature at 'building' status so the job flows through: building → combining → verifying → pr_ready.

BEGIN;

-- 1) Replace request_standalone_work: auto-create feature when p_feature_id IS NULL.
...
BEGIN
  ...
  -- Idempotency: no active duplicate role+feature.
  SELECT id
...
-- 2) Replace all_feature_jobs_complete: include ALL jobs (remove source filter).
