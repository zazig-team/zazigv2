...
-- Date: 2026-04-01
-- Purpose: Ensure completed_features in pipeline snapshot reflects most recently
-- completed/promoted work first and keeps the intended top-10 window.
    ...
    'completed_features', COALESCE((
      SELECT jsonb_agg(
        jsonb_build_object(
          ...
          'promoted_version', recent.promoted_version
        )
        ORDER BY recent.completion_sort_at DESC, recent.updated_at DESC, recent.created_at DESC
      )
      FROM (
        SELECT
          f.id,
          ...
          f.created_at,
          f.promoted_version,
          COALESCE(f.completed_at, f.updated_at, f.created_at) AS completion_sort_at
        FROM public.features f
        WHERE f.company_id = p_company_id
          AND f.status = 'complete'
        ORDER BY COALESCE(f.completed_at, f.updated_at, f.created_at) DESC, f.updated_at DESC, f.created_at DESC
        LIMIT 10
      ) recent
    ), '[]'::jsonb),
