...
Ask which Codex model to use. Options:
...
If the user mentions a Trello board, include it. You can check `~/Documents/GitHub/trw-projects/AGENTS.md` for known Trello board IDs (shared reference section in the router file).

Show the generated HEARTBEAT.md to the user and confirm before writing.

### Step 12: Generate Operating Manual

**This step is mandatory.** Every exec must have an operating manual at `~/Documents/GitHub/trw-projects/{ROLE}-AGENTS.md`. This is the canonical reference that gets symlinked into the agent's context and loaded as part of the system prompt.

**Naming convention:**
- CPO -> `CPO-AGENTS.md`
- CMO -> `CMO-AGENTS.md`
- VP Engineering -> `VP-ENG-AGENTS.md`
- CoS -> `COS-AGENTS.md`
- VP Product (Marginally) -> `VP-PRODUCT-MARGINALLY-AGENTS.md`
...
You operate as a persistent Slack bot (@{role_id}) via the Codex Agent SDK.
...
ln -sf ~/Documents/GitHub/trw-projects/{ROLE}-AGENTS.md ~/.chainmaker/agents/{role_id}/context/{role_id}-operating-manual.md

# For project-scoped agents:
ln -sf ~/Documents/GitHub/trw-projects/{ROLE}-AGENTS.md {project_path}/.chainmaker/agents/vp-product/context/{role_id}-operating-manual.md
```
...
**Focus project board IDs** (from AGENTS.md router, shared reference section):
...
- Operating manual: ~/Documents/GitHub/trw-projects/{ROLE}-AGENTS.md
...
- **Operating manual is mandatory** — every exec gets a `{ROLE}-AGENTS.md` in trw-projects/
