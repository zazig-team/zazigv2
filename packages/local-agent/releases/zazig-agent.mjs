const AGENT_BUILD_HASH = "a676dbc8";
...
var RepoManager = class {
      ...
      if (existsSync3(repoDir)) {
        try {
          ...
          if (isBare === "true") {
            ...
            rmSync2(repoDir, { recursive: true, force: true });
          }
        } catch {
          rmSync2(repoDir, { recursive: true, force: true });
        }
   ...
   */
  async fetchForExpertSession(projectName, _sessionId) {
    const cloneDir = join3(REPOS_BASE, projectName);
    return this.withLock(cloneDir, async () => {
      await this.git(cloneDir, "fetch", "origin");
  ...
  async handleStartExpert(msg) {
    ...
    if (needsRepo && msg.project_id && msg.repo_url) {
      try {
        ...
        await execFileAsync5("git", ["-C", cloneDir, "worktree", "prune"]);
...
`);
  ...
  async pushUnpushedCommits(session) {
    ...
    if (!session.repoDir || !session.cloneDir || !expertBranch || !session.startCommit)
      return;
    try {
