-- 118_pipeline_snapshot_failed_jobs_flag.sql
-- Date: 2026-03-09
-- Purpose: Add has_failed_jobs flag to each feature object in
--          refresh_pipeline_snapshot() features_by_status output.
      ...
      FROM (
        SELECT f.status, jsonb_agg(
          jsonb_build_object(
            ...
            'has_failed_jobs', EXISTS (
              SELECT 1
              FROM public.jobs j2
              WHERE j2.feature_id = f.id
