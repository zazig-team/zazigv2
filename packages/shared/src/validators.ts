...
export function isStartJob(v: unknown): v is StartJob {
  ...
  if (!["code", "infra", "design", "research", "docs", "persistent_agent", "verify", "breakdown", "combine", "merge", "deploy_to_test", "deploy_to_prod", "review", "bug", "feature_test", "ci_check"].includes(v.cardType as string)) return false;
