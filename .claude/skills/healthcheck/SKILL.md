...
2. Check for legacy `claude-*` tmux sessions — assume `tom` instance
...
| Session alive, at welcome screen | `[auto]` | Send startup prompt via `claude-send` |
...
```bash
claude-send {instance-id}-{role} "Read manuals/{ROLE}-CLAUDE.md and ~/.zazig/instances/{instance-id}/context.md — you are the {Role} agent. Start your execution loop."
```

Role → manual mapping:
| Role | Manual file | Prompt role name |
|------|------------|-----------------|
| supervisor | SUPERVISOR-CLAUDE.md | Supervisor |
| vp_eng | VP-ENG-CLAUDE.md | VP-Engineering |
| cpo | CPO-CLAUDE.md | CPO |
| cto | CTO-CLAUDE.md | CTO |
| cmo | CMO-CLAUDE.md | CMO |
...
- **Using `tmux send-keys` instead of `claude-send`** — Claude Code TUI swallows raw send-keys. Always use `claude-send`.
...
- **Killing sessions from inside tmux** — if you're running in a tmux session yourself, `tmux kill-session` on your own session would kill you. The skill runs from Claude Code which may or may not be in tmux — always check.
