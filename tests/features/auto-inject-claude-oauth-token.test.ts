...
// ---------------------------------------------------------------------------
// Fix: ANTHROPIC_API_KEY must NOT be injected into daemon env
// (injecting a stale keychain token caused "Invalid API key" failures;
//  Claude Code manages its own OAuth refresh natively via `claude login`)
// ---------------------------------------------------------------------------

describe('start.ts — ANTHROPIC_API_KEY NOT injected into daemon environment', () => {
  const FILE = 'packages/cli/src/commands/start.ts';
  let content: string | null;
  ...
  it('does not call readClaudeTokenFromKeychain() inside the start() function', () => {
    // The function definition may remain, but must not be called in start()
    // so that no stale token is ever injected into the daemon env.
    const startFnBody = content?.match(/export async function start\(\)[^]*$/)?.[0] ?? '';
    expect(startFnBody).not.toMatch(/readClaudeTokenFromKeychain\(\)/);
  });

  it('does not set ANTHROPIC_API_KEY in the daemon env block', () => {
    // Injecting a keychain token as ANTHROPIC_API_KEY is harmful — the token
    // becomes stale and Claude Code jobs fail with "Invalid API key".
    // Claude Code uses its own OAuth session (claude login) with auto-refresh.
    expect(content).not.toMatch(/ANTHROPIC_API_KEY/);
  });

  it('does not spread a claudeToken into the daemon env', () => {
    // No conditional spread of claudeToken should appear in start()
    expect(content).not.toMatch(/claudeToken\s*\?.*ANTHROPIC_API_KEY|ANTHROPIC_API_KEY.*claudeToken/);
  });
});
