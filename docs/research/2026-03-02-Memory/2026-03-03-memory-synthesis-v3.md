# Memory Architecture Synthesis v3: Definitive Recommendation

**Date:** 2026-03-03
**Author:** Merged synthesis (Claude Opus 4.6 + Gemini CLI + OpenAI Deep Research), commissioned by Tom
**Sources:** 22 research documents (4 deep research, 13 tweet captures, 5 repo recons), ORG MODEL reference, two independent synthesis reports, one OpenAI deep research report
...
This v3 incorporates the OpenAI deep research report (`2026-03-02-Memory-deep-research-openai.md`), which was produced independently from the same research brief but not included in the original v2 synthesis. Changes are **modest but real** -- the OpenAI report independently arrives at compatible recommendations, strengthening confidence in v2's positions while adding specific academic grounding and a few concrete additions.

**New material added:**
- **Section 1**: Added the "forms" taxonomy (token-level, parametric, latent) from 2025 surveys, and the "functional vs structural" taxonomy debate from the 2026 MAG survey. Added CoALA framework as the canonical reference for modular memory components. Neither changes our architecture, but both strengthen the theoretical foundation.
- **Section 2**: Added A-MEM's Zettelkasten-inspired dynamic note linking as a Tier 3 innovation. Added OpenAI's `/responses/compact` as a first-class compaction primitive reference.
- **Section 5**: Added explicit Memory-to-Doctrines/Canons/Skills contracts (v2 had this implicitly in Section 8.6; now formalised in the lifecycle section based on the OpenAI report's crisp framing).
- **Section 7**: Added the "system-level costs are frequently overlooked" warning from the 2026 MAG survey as a concrete risk to plan against.
- **Section 8**: Added LongMemEval's 5 evaluation axes and LoCoMo benchmark as specific evaluation targets. Added explicit evaluation subsection.
- **Section 9**: Added latent/role-aware memory as a Phase 4 research direction (beyond Phase 3).
- **Appendix A**: Added OpenAI deep research to source table.

**Strengthened positions (no change to recommendation, but stronger evidence):**
- "Most stored memories are never retrieved" -- independently confirmed by OpenAI report's framework survey
- Hybrid baseline (structured truth + episodic log + hybrid retrieval) -- independently recommended
- Memory writes as privileged operation -- independently arrived at the same conclusion
- Three-ring privacy model -- OpenAI's private/team/company rings are almost identical to v2's scoping
- Temporal KG as optional upgrade, not starting point -- independently recommended as Phase 4

**Contradictions found: None material.** The OpenAI report is less opinionated than v2 but arrives at compatible recommendations. Minor framing difference: OpenAI describes a "four-tier memory plane" (session state, core identity, company semantic, episodic archive) which describes tiers of access speed. V2's 8-type taxonomy describes types of content. These are complementary, not contradictory.

---

## How This Document Was Made

Three independent synthesis efforts were produced:
...
- **OpenAI deep research** (~437 lines): Framework-by-framework comparison table, academic taxonomy survey, cold-start flow diagram, explicit Memory-to-Doctrines/Canons/Skills contracts, evaluation benchmarks.

V2 merged the Claude and Gemini syntheses. V3 incorporates the OpenAI report into that merged document. Where all three agree, confidence is highest. Where the OpenAI report adds new material, it is woven into the relevant section with attribution.
...
Both the Claude and Gemini syntheses identify the same progression, which Gemini frames crisply:
...
### Taxonomy Landscape (v3 addition)

The OpenAI report surfaces an important academic context for how memory systems are being classified in 2025-2026 research. Three competing taxonomy lenses exist:

1. **Functional taxonomies (what memory is for):** Recent surveys argue that "short-term vs long-term" is inadequate and propose breaking memory down by function -- factual vs experiential vs working -- alongside formation/evolution/retrieval dynamics. This aligns with our 8-type taxonomy, which is fundamentally functional.

2. **Structural taxonomies (how memory is organised):** A 2026 survey of "MAG systems" (memory-augmented generation) categorises systems by memory structure: lightweight semantic stores, entity-centric/personalised memory, episodic/reflective memory, and structured/hierarchical memory. Critically, this survey highlights that **system-level constraints (latency, throughput, maintenance cost) are frequently overlooked** -- a warning we take seriously in our phased implementation.

3. **Form taxonomies (how memory is represented):** A 2025 survey frames memory "forms" as:
   - **Token-level memory:** Explicit stored text retrieved into context (this is what v1-v3 of Zazig builds)
   - **Parametric memory:** Information learned into model weights (not applicable to us -- we use external models)
   - **Latent memory:** Compact learned representations (not plain text) used for retrieval/conditioning

   For multi-agent systems, recent work proposes **role-aware latent memory** to reduce homogenisation and information overload across agents. This is relevant for Phase 4+ when we may want the CTO's memory retrieval to be fundamentally shaped differently from the CPO's, not just filtered differently.

The canonical academic reference for modular memory components is **CoALA** (Cognitive Architectures for Language Agents), which separates memory modules from the action space and treats procedural memory as developer-supplied rather than freely mutable. Our architecture aligns with CoALA's structure: doctrines are immutable policy, skills are controlled procedures, and memory is the mutable state layer.

**Practical impact on Zazig:** None of these taxonomies change our architecture. Our 8-type taxonomy is functional (correct approach for an orchestrator that needs to decide what to inject). Token-level memory is the right form for Phases 1-3. Latent memory is a Phase 4+ research direction. The MAG survey's warning about system-level costs is incorporated into our risk planning.
...
The OpenAI report adds a useful framework-by-framework feature matrix across LangGraph, CrewAI, AutoGen, OpenAI Agents SDK, Letta, MemGPT, and Zep/Graphiti. The key additional details worth noting:

- **AutoGen Teachability**: Persists "memos" in a vector DB and retrieves them as needed, explicitly avoiding copying all memory into context. A concrete implementation of the progressive disclosure pattern we already plan.
- **OpenAI Agents SDK**: Has first-class **compaction primitives** including a `/responses/compact` endpoint and a documented "state management with long-term memory notes" cookbook that demonstrates end-of-run consolidation with dedup and conflict handling. This validates our compaction flush pattern (Section 5.1, Path 3).
- **Letta**: All conversations share memory blocks but keep context windows separate to avoid "unrelated messages polluting another context." Read-only blocks are supported -- directly validates our doctrine-as-immutable pattern.

### The Critical Consensus Finding

All three synthesis efforts converge on the same fundamental insight: **most stored memories are never retrieved.** Zep retrieves top-20 facts per query; Mem0 defaults to top-3. The OpenAI report independently confirms this through its framework survey. Retrieval quality matters exponentially more than storage capacity. This shapes the entire architecture: optimise for recall precision, not memory volume.

---

## 2. Key Innovations Worth Stealing

All three synthesis efforts identified overlapping sets of innovations. The merged ranking below takes the best framing from each, sorted into must-have, high-value, and worth-noting tiers.
...
| 5 | **Compactor-as-Memory-Extractor** | Spacebot, claude-code-semantic-memory | When context compacts, extract memories in the same LLM pass. The "memory flush" that captures information before it leaves the context window. Critical for persistent agents (CPO) that hit compaction regularly. OpenAI report confirms this via the Agents SDK's first-class compaction primitives (`/responses/compact`). |
...
| 14 | **End-of-Run State Consolidation** | OpenAI Agents SDK cookbook | Consolidate session notes into global notes with dedup and conflict resolution at the end of each run, then inject shaped state at the next run. This is the production-validated version of our compaction flush + post-job extraction pipeline. |
...
| 19 | **QMD for Local Knowledge Search** | @andrarchy, @code_rams, @intellectronica | BM25 + vector embeddings for local markdown files. 96% token savings. Relevant for the local-agent daemon side. |
| 20 | **A-MEM Zettelkasten-Inspired Dynamic Linking** | A-MEM paper (v3 addition) | Dynamically organised notes where new memories trigger updates to existing contextual descriptions and attributes. A "memory evolves" mechanism that goes beyond simple supersession -- existing memories get richer as new related memories arrive. Worth studying for Phase 3's procedural synthesis, where episodic sequences should enrich existing procedure memories rather than just creating new ones. |
...
The OpenAI report independently arrives at the same recommendation: a "hybrid default" of structured DB for truthy state + governance, vector/hybrid retrieval for long-tail recall, with optional temporal graph as an upgrade. It frames this as the "pragmatic baseline" for medium enterprise scale, which is exactly our position.
...
| **Compaction flush** | Build, informed by OpenAI Agents SDK `/responses/compact` pattern | OpenAI SDK cookbook |
...
The OpenAI Agents SDK validates this pattern with first-class compaction primitives (the `/responses/compact` endpoint and documented compaction guides). Their cookbook explicitly demonstrates end-of-run consolidation into global notes with dedup and conflict handling -- the same pattern we implement here.
...
### 5.7 Memory-to-Knowledge-Stack Contracts (v3 addition)

The OpenAI report frames the relationship between memory and the rest of Zazig's knowledge stack with explicit contracts. This was implicit in v2's Section 8.6 (priority hierarchy) but deserves formal treatment:

**Memory -> Doctrines: NEVER WRITE.** Doctrines are normative beliefs. Memory is empirical experience. The memory system can observe that a doctrine exists and should not contradict it, but it cannot modify doctrines. Promotion to doctrine requires the explicit pipeline in Section 5.8.

**Memory -> Canons: PROPOSE ONLY.** Memory can propose updates to canon material (curated reference knowledge) through the promotion pipeline, but only after validation. This aligns with benchmark emphasis on knowledge updates requiring verification.

**Memory -> Skills: GATED UPDATES.** When episodic memory reveals a better procedure ("every time I did X before Y, the test suite passed"), this is a candidate for skill improvement -- but requires code review, human approval, or offline evaluation. CoALA's architecture explicitly treats procedural memory as developer-supplied, not freely mutable, and we follow this.

**Enforcement:** The orchestrator is the gatekeeper. Memory retrieval and injection happen at orchestrator dispatch time (Section 8.3). The orchestrator checks memory-doctrine conflicts before injection and suppresses memories that contradict active doctrines. Flagged conflicts are surfaced for human review, not silently resolved.

### 5.8 Memory Promotion Pipeline
...
The OpenAI report independently recommends a three-ring model (private, team, company) for multi-agent shared memory, motivated by context pollution risks, correctness requirements, and auditability. This maps directly to our scoping: Individual = private ring, Project/Feature/Role-Shared = team ring, Company = company ring. The independent convergence strengthens confidence in this design.
...
The OpenAI report reinforces this with the Letta pattern: all conversations share memory blocks but keep context windows separate. Agents read shared state but never see each other's raw conversation history. This is the same boundary we enforce.
...
Both the Claude and Gemini syntheses agree that Spacebot is the current state of the art for open-source agent memory. Both also agree that Spacebot's memory is, by the community's own admission, "average" despite impressive architecture. The question is: why is it average, and how does Zazig avoid the same fate?
...
### The System-Level Cost Risk (v3 addition)

The 2026 MAG (Memory-Augmented Generation) survey surfaces a warning that neither the Claude nor Gemini syntheses emphasise enough: **system-level costs of memory maintenance are frequently overlooked.** Teams build sophisticated memory extraction, consolidation, and retrieval pipelines, then discover that the latency and token overhead make their agents slower and more expensive than agents without memory.

This is a real risk for Zazig. Our memory pipeline adds LLM calls at:
- Write time: extraction prompt (~500 tokens in, ~200 tokens out per job)
- Maintenance time: consolidation, reflection, conflict resolution
- Read time: bulletin synthesis, ContextPack assembly
- Mid-task: PreToolUse memory refresh

**Mitigation:** Phase 2 must include instrumentation for memory operation latency, token costs, and retrieval precision. If the bulletin synthesis adds 2 seconds to every prompt and the CPO already has 8-second response times, the memory system is net-negative regardless of recall quality. The token economics tracking from claude-mem (Innovation #17) becomes a Phase 2 deliverable, not Phase 3.
...
1. DISPATCH-TIME ASSEMBLY (All agents)
   ...
   -> Memory-Doctrine conflict check (suppress memories that contradict active doctrines, flag for review)
...
7. COST TRACKING: Log token costs for extraction, retrieval, synthesis (v3 addition -- moved from Phase 3 to Phase 2)
8. REFLECTION (Phase 3): Generate Gap Questions from unresolved observations
```
...
**Memory-to-knowledge-stack contracts (formalised in Section 5.7):** Memory never writes to doctrines. Memory proposes to canons (gated). Memory suggests skill improvements (gated). The orchestrator enforces all gates.
...
### 8.8 Evaluation Framework (v3 addition)

The OpenAI report surfaces specific benchmarks and metrics that should be our evaluation targets. Without these, we risk building a system that feels good but cannot be measured.

**Academic benchmarks to test against:**
- **LongMemEval**: Tests 5 specific axes -- extraction, multi-session reasoning, temporal reasoning, knowledge updates, and abstention (knowing when you don't know). The abstention axis is particularly important for Zazig: agents must decline to answer rather than confabulate when memory is insufficient.
- **LoCoMo**: Tests very long multi-session conversations. Evaluates QA accuracy, event summarisation, and long-range dynamics. Relevant for the CPO, which will accumulate months of conversation history.
- **DMR (Deep Memory Retrieval)**: Established by MemGPT. Zep reports results relative to MemGPT on this benchmark. Our baseline should be competitive.

**Production metrics (from v2, now prioritised as Phase 2 deliverables):**
...
- User-visible regressions (wrong recall, stale decisions, privacy leaks)
- **Token cost per memory operation** (extraction, retrieval, synthesis) -- the MAG survey's warning operationalised
...
**Goal:** Semantic search, memory graph, decay, bulletin pattern, confidence scoring, PreToolUse injection, cost instrumentation.
...
13. **Token cost instrumentation** -- track cost per extraction, retrieval, and synthesis operation (v3: moved from Phase 3)
14. **Memory operation latency tracking** -- instrument all memory pipeline steps for SLO monitoring (v3 addition, per MAG survey warning)

**What this gives you:** Semantic retrieval that actually finds relevant memories. The CPO gets a coherent briefing synthesised from its accumulated knowledge. Old memories fade naturally. Contradictions are detected and resolved (not silently accumulated). Mid-task memory refresh prevents workflow drift. You know what memory costs and whether it is worth it.
...
10. **Token economics tracking** -- full ROI analysis: memory creation cost vs retrieval savings (builds on Phase 2 instrumentation)
...
### Phase 4: Research Directions (beyond Phase 3) (v3 addition)

These are not planned deliverables but research directions informed by the academic landscape:

1. **Latent memory representations.** Instead of storing memories as explicit text retrieved into context (token-level memory), explore compact learned representations that condition retrieval without consuming token budget. The 2025 survey on memory forms identifies this as a frontier direction. Relevance for Zazig: as memory volume grows into the tens of thousands, token-level injection may become a bottleneck even with progressive disclosure.

2. **Role-aware memory retrieval.** Recent work proposes that multi-agent systems should have retrieval shaped by agent role, not just filtered by it. The CTO's retrieval model would inherently prioritise technical decisions and architectural patterns, while the CPO's would prioritise product strategy and user signals -- without requiring different filter rules for each. This goes beyond our current scope-based filtering.

3. **Temporal knowledge graph upgrade.** If our Postgres-based graph and associations prove insufficient for "what was true when" reasoning at scale, a Zep/Graphiti-style temporal KG with validity intervals becomes the upgrade path. This is explicitly deferred because the infrastructure cost is high and our graph will be small enough for Postgres in year one.

4. **A-MEM-style dynamic memory evolution.** Instead of memories being static once written (with only supersession as the update mechanism), explore memories that enrich themselves as related memories arrive -- Zettelkasten-style linking where each memory's contextual description grows over time.
...
| **OpenAI deep research** | **Deep research (v3 addition)** | **Framework-by-framework comparison table, academic taxonomy survey (functional/structural/form), Memory-to-knowledge-stack contracts, LongMemEval/LoCoMo/DMR benchmark mapping, MAG system-level cost warning, A-MEM dynamic linking, cold-start flow diagram** |
...
| Memory-to-knowledge-stack relationship (v3) | Explicit contracts (never write doctrines, propose-only canons, gated skills) | Implicit/undocumented boundary | OpenAI report's framing makes the contracts actionable; prevents "memory creep" into normative layers |
| Cost instrumentation timing (v3) | Phase 2 (with intelligence features) | Phase 3 (deferred) | MAG survey evidence that system-level costs sink memory systems; must measure before scaling |
| Latent memory (v3) | Phase 4 research direction | Phase 3 deliverable, or ignored | Promising but adds ML complexity; token-level memory sufficient for year one |
...
| Doctrine vs Memory boundary | Doctrines win conflicts; promotion pipeline surfaces memory candidates for doctrine review; explicit contracts formalised in Section 5.7 |
