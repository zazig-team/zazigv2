...
server.tool(
  ...
  "Create a project rule that will be automatically injected into future agent prompts. Use when you identify a preventable pattern that future agents should know about.",
  {
    ...
    rule_text: z.string().describe("The rule in plain language — what agents should do or avoid"),
    applies_to: z.array(z.string()).describe("Job types this rule applies to, e.g. ['code', 'combine', 'test']"),
  },
  guardedHandler("create_project_rule", async ({ project_id, rule_text, applies_to }) => {
    ...
    if (!jobId) {
      return {
        content: [{ type: "text" as const, text: "Error: ZAZIG_JOB_ID is required for create_project_rule" }],
        isError: true,
      };
    }
